...
$cattle_owner_uid = $data['ownerUid'];
// THIS IS VEEEEEEEEEEERY DUMB (and quick). PLEASE DO NOT DO THIS ON A SERIOUS PRODUCTION APPLICATION
// Wesley from the future, if you choose to do this as a serious thing, please, please, for the love of God, figure a better way to do this
$fields = $data['fields'];

if ($cattle_owner_uid) {
    // Get data from owner username - get all the entries associated with it
    $query = "SELECT $fields FROM cattle WHERE owner = $cattle_owner_uid";
} else {
    $uid = $data['uid'];
    $query = "SELECT $fields FROM cattle WHERE UID = $uid";
}


$cattle_info_result = mysqli_query($mysqli_connection, $query);
$cattle_array = array();
